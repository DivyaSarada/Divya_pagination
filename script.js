...
list.setAttribute(
  "class",
  "pagination justify-content-center pagination-wrap d-flex"
);
...
lprev.setAttribute("class", "page-link");
lprev.setAttribute("style","background-color:gray;color:white")

function previous(j) {
  if (j != 0) {
    lprev.setAttribute(
      "href",
      "javascript:page(" + j + ");previous(" + (j - 1) + ")"
    );
  }
}
lprev.innerHTML = "Previous";
prev.append(lprev); 

for (var i = 1; i <= 10; i++) {
  var rem = document.createElement("li");
  rem.setAttribute("class", "page-item");
  list.append(rem);
  var rem1 = document.createElement("a");
  rem1.setAttribute("class", "page-link");
  rem1.innerHTML = i;
  rem.append(rem1);
  rem1.setAttribute("href","javascript:page(" +  i +  ");previous:(" +  (i - 1) +  ");next(" +  (i + 1) +  ");" );
}
var nex = document.createElement("li");
nex.setAttribute("class", "page-item");
list.append(nex);
var lnext = document.createElement("a");
lnext.setAttribute("class", "page-link");
lnext.innerHTML = "Next";
lnext.setAttribute("style","background-color:gray;color:white")
...
col.setAttribute("class", "col-10 offset-1");
row.append(col);

var table = document.createElement("table");
table.setAttribute("class", "table table-bordered table-dark ");
table.setAttribute("style","border:thick solid gray;text-align:center")
table.setAttribute("id", "myTable");
col.append(table);
var tr = document.createElement("tr");
tr.setAttribute("class", "table-success text-dark");
table.append(tr);
var id1 = document.createElement("th");
id1.setAttribute("style","background-color:gray;color:white")
id1.innerHTML = "ID";
var name1 = document.createElement("th");
name1.setAttribute("style","background-color:gray;color:white")

name1.innerHTML = "NAME";
var email = document.createElement("th");
email.innerHTML = "EMAIL";
email.setAttribute("style","background-color:gray;color:white")

tr.append(id1, name1, email);
for (let i = 0; i < 10; i++) {
  let tr1 = document.createElement("tr");
  tr1.setAttribute("class", "table-secondary text-dark");
  table.append(tr1);
  ...
  lnext.setAttribute("href", "javascript:previous(4);");
}
function page(i) {
  for (let j = (i - 1) * 10; j < (i-1)*10 + 10; j++) {
    
    lnext.setAttribute("href", "javascript:next(" + (i + 1) + ");");
   
    lprev.setAttribute("href", "javascript:previous(" + (i - 1) + ");");
    document.getElementById("myTable").deleteRow(1);
 
    var rows = document.createElement("tr");
    rows.setAttribute("class", "table-secondary text-dark");
    table.append(rows);
    var c1 = document.createElement("td");
    c1.innerHTML = obj[j].id ;
           var c2 = document.createElement("td");
    c2.innerHTML = "" + obj[j].name + "";
    var c3 = document.createElement("td");
    c3.innerHTML = "" + obj[j].email + "";
    
    rows.append(c1, c2, c3);
  }
}
document.body.append(container, list);
